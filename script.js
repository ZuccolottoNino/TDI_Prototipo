...
const secciones = [
  ...
  },
  {
    ...
    titulo: "Calidad vs Tiempo: Comparativa multidimensional entre modelos de IA",
    badges: ["5 MODELOS DE FRONTERA", "5 DIMENSIONES VERIFICADAS", "BENCHMARKS 2025-2026"],
    resumenCorto: "Evaluación multidimensional de Claude Opus 4.7, GPT-5.5, Gemini 3.1 Pro, Grok 4.6 y DeepSeek V4 Pro en corrección, seguridad, velocidad, precio y contexto.",
    contexto: "Este gráfico radial resume la relación técnica entre velocidad, costo y calidad de software al programar con los 5 principales modelos de inteligencia artificial de frontera: Claude Opus 4.7, GPT-5.5, Gemini 3.1 Pro, Grok 4.6 y DeepSeek V4 Pro. A partir de un pentágono comparativo normalizado (escala 0 a 10), se analizan cinco métricas empíricas clave: Corrección Funcional (% de código generado que pasa todos los tests en BaxBench), Vulnerabilidad (% de código generado seguro sin exploits), Velocidad (throughput de inferencia medido en tokens por segundo), Precio (tarifa blended promedio ponderada entre input y output en USD/M) y Contexto (amplitud de ventana en tokens para ingesta de proyectos y repositorios enteros). El pentágono permite identificar los compromisos arquitectónicos reales: modelos con mayor corrección funcional demandan mayor latencia y costo por token, mientras que arquitecturas de alta velocidad y costo ultrabajo exhiben mayores brechas en seguridad y retención de contexto.",
    callout: {
      tipo: "verificado",
      icono: "⚡",
      header: "MÉTRICAS TÉCNICAS VERIFICADAS // 5 DIMENSIONES",
      texto: "Todos los datos del pentágono corresponden a benchmarks formales y mediciones de infraestructura verificadas: Corrección y Código Seguro provienen del leaderboard oficial de BaxBench (392 tareas en 14 frameworks) y AutoBaxBench Difícil; Velocidad refleja mediciones de throughput de salida continua (tok/s); Precio sintetiza las tarifas blended oficiales de entrada y salida ($/M); y Contexto cuantifica la ventana de tokens disponible para desarrollo a gran escala."
    },
    kpis: [
      { valor: "86.2%", etiqueta: "MÁXIMA CORRECCIÓN FUNCIONAL (CLAUDE OPUS 4.7)" },
      { valor: "90 tok/s", etiqueta: "MÁXIMO THROUGHPUT DE SALIDA (GEMINI 3.1 PRO)" },
      { valor: "$5.22/M", etiqueta: "MENOR PRECIO BLENDED INPUT/OUTPUT (DEEPSEEK V4 PRO)" }
    ],
    tldr: [
      { num: "01", tit: "Compromisos de Frontera", desc: "Ningún modelo lidera en simultáneo las 5 dimensiones: Claude Opus 4.7 alcanza la mayor corrección (86.2%) pero con menor velocidad (42 tok/s) y mayor costo ($30.00/M), mientras Gemini 3.1 Pro domina en throughput (90 tok/s) con menor tasa de corrección (49.8%)." },
      { num: "02", tit: "La Brecha de Seguridad", desc: "Superar pruebas unitarias no garantiza seguridad: entre el 23.1% (GPT-5) y el 42.0% (Grok 4) del código funcional contiene vulnerabilidades explotables ante exploits automatizados en BaxBench." },
      { num: "03", tit: "Escala y Eficiencia Económica", desc: "Las ventanas de contexto alcanzan entre 500K y 1.1M tokens (lideradas por GPT-5.5 con 1.1M), mientras que en costos DeepSeek V4 Pro ($5.22/M) y Grok 4.6 ($8.00/M) reducen drásticamente la inversión frente a GPT-5.5 ($35.00/M)." }
    ],
    metodologia: [
      { clave: "EVALUACIÓN_DE_CÓDIGO", valor: "BaxBench Oficial (392 tareas sobre 14 frameworks) & AutoBaxBench Difícil" },
      { clave: "VELOCIDAD_DE_INFERENCIA", valor: "Throughput medido en tokens por segundo (tok/s) en APIs y clústeres de inferencia" },
      { clave: "PRECIO_Y_CONTEXTO", valor: "Tarifas blended ponderadas (input/output promedio por 1M de tokens) y ventanas de 500K a 1.1M tokens" }
    ]
  },
  {
    id: "fin-del-teclado",
...
function formatearTextoReadme(texto) {
  if (!texto) return "";
  return texto
    .replace(/(Claude Opus 4\.7|Claude Opus 4\.5 Thinking|GPT-5\.5|GPT-5|DeepSeek V4 Pro|DeepSeek R1|Grok 4\.6|Grok 4|Gemini 3\.1 Pro|Géminis 2\.5 Pro|Claude 4\.5|Gemini 3 Pro)/g, '<span class="readme__chip-inline">$1</span>')
    .replace(/(AutoBaxBuilder|AutoBaxBench|BaxBench|Vibe Coding|Andrej Karpathy)/g, '<span class="readme__chip-inline">$1</span>')
...
function renderizarReadme(seccion) {
  ...
  const calloutHtml = seccion.callout ? `
    <div class="readme__callout ${seccion.callout.tipo ? `readme__callout--${seccion.callout.tipo}` : ''}">
      <div class="readme__callout-icono">${seccion.callout.icono || '⚡'}</div>
      <div>
...
document.addEventListener("keydown", (e) => {
  ...
  if (e.key === "ArrowUp" && graficoActivo.onFlechaVertical) graficoActivo.onFlechaVertical(-1);
  if (e.key === "ArrowDown" && graficoActivo.onFlechaVertical) graficoActivo.onFlechaVertical(1);
});
